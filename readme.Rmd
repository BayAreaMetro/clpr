...
Combine transactions so each row reflects a complete ride

```{r}
complete_rides <- drop_tagons(transactions_df)
```

Create a dataframe summarizing transfers within a given time window (in minutes)

```{r}
transfer_df <- create_transfer_df(complete_rides, 120)
```
