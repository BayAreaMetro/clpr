...
#### Filter #1
```{r shiny_plot_onefilter, echo=FALSE, cache=FALSE}
# 
# textInput("Filter01", "Enter comma-separated sequence (up to 5) to filter:", ",")
# 
# renderPlot({
#   
#   sequenceList <- data.frame(strsplit(input$Filter01, ","))
#   sequence01   <- as.character(sequenceList[1,1])
#   sequence02   <- as.character(sequenceList[2,1])
#   
#   chart.data.2 <- input.seq %.%
...
#     summarise(Cards = sum(n))
#   
#   chart_title <- paste("Sequences with ", "'", sequence01, "'", " and ", "'", sequence02, "'")
# 
#   plot.2 <- ggplot(chart.data.2, aes(x = DayOfMonth, y = Cards)) + geom_point(size = 4, colour = "#D55E00") + xlab("Day of April 2013") + ylab("Clipper Cards Used") + labs(title = chart_title) + theme(text = element_text(size=15), axis.text.x = element_text(vjust=1), axis.title.y = element_text(vjust=2.0), legend.position="none")
#   
#   print(plot.2)
#   
#   
...
#### Filter #2
```{r shiny_plot_twofilter, echo=FALSE, cache=FALSE}

# textInput("Filter01", "Sequences that contain:", " ")
# textInput("Filter02", "And contain:", " ")
# 
# renderPlot({
#   
...
#     summarise(Cards = sum(n))
#   
#   chart_title <- paste("Sequences with ", "'", input$Filter01, "'", " and ", "'", input$Filter02, "'")
# 
...
#### Filter #5
```{r shiny_plot_fivefilter, echo=FALSE, cache=FALSE}

textInput("Filter01", "Sequences that contain:", " ")
textInput("Filter02", "And also contain:", " ")
textInput("Filter03", "And also contain:", " ")
textInput("Filter04", "And also contain:", " ")
textInput("Filter05", "And also contain:", " ")
...
renderPlot({
  ...
  chart.data <- input.seq %.%
    ...
    mutate(WorkingSequence = gsub(input$Filter02, "", WorkingSequence)) %.%
    filter(grepl(input$Filter03, WorkingSequence)) %.%
    mutate(WorkingSequence = gsub(input$Filter03, "", WorkingSequence)) %.%
    filter(grepl(input$Filter04, WorkingSequence)) %.%
    mutate(WorkingSequence = gsub(input$Filter04, "", WorkingSequence)) %.%
    filter(grepl(input$Filter05, WorkingSequence)) %.%
    mutate(DayOfMonth = substr(CircadianDate,9,10)) %.%
    group_by(DayOfMonth) %.%
    summarise(Cards = sum(n))
  
  chart_title <- paste("Sequences with ", "'", input$Filter01, "'", " and ", "'", input$Filter02, "'", " and ", "'", input$Filter03, "'", " and ", "'", input$Filter04, "'", " and ", "'", input$Filter05, "'")
