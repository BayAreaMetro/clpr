...
QUERY_STATEMENT = paste("SELECT
                        ...
                        FROM [gis].[Clipper].[ThreeRandomWeeksAllMonths]
                        WHERE (Month = ",MONTH_SELECT,") AND (Year = ",YEAR_SELECT,")")
...
df_raw <- df_raw %>%
  mutate(CircadianDayOfWeek_Name = as.character(CircadianDayOfWeek_Name)) %>%
  mutate(TagOnTime_Time          = as.character(TagOnTime_Time)) %>%
  mutate(TagOffTime_Time         = as.character(TagOffTime_Time))

working_date <- df_raw %>%
  select(Year, Month, CircadianDayOfWeek, CircadianDayOfWeek_Name, RandomWeekID) %>%
